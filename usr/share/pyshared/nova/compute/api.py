...
class API(BaseAPI):
    ...
    def _delete(self, context, instance):
        ...
        con = None
        try:
            # Now remove instance from PLACEMENTS db table
            con = mdb.connect('host', 'username', 'password', 'db_name')
            cur = con.cursor()

            # Insert into termination request into REQs_TERMINATE table
            cur.execute("INSERT INTO REQs_TERMINATE(time, uuid) VALUES('" + str(datetime.now()) + "', '"  + str(instance_id) + "')")
            con.commit()

            # Remove this uuid record from SCH_PLACEMENTS
            cur.execute("DELETE FROM SCH_PLACEMENTS WHERE req_id = '"  + str(instance_id) + "'") 
            con.commit()
